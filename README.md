...
L0013 is a Graffiticode dialect — the first child of [@graffiticode/l0000](https://www.npmjs.com/package/@graffiticode/l0000). It inherits L0000's base functional vocabulary (arithmetic, lists, lambdas, `map`/`filter`/`reduce`, pattern matching, tags) and adds a small set of UI primitives for rendering hello greetings, images, and themed output.

## Vocabulary

L0013 adds the following on top of the L0000 base lexicon:
...
- **`packages/core`** — `@graffiticode/l0013`: the language itself (lexicon, checker, transformer). Pure TypeScript, depends on `@graffiticode/l0000`.
- **`packages/api`** — `@graffiticode/api-l0013`: the L0013 language server. Express app exposing `/compile`, `/form`, and static assets. Runs on port `50013`.
- **`packages/view`** — `@graffiticode/l0013-view`: the React view component (Form) used to render compiled output. Built with Vite + Tailwind, layered on top of `@graffiticode/l0000-view`.
...
# Start the dev server (API on :50013, Firestore emulator on :8080)
npm run dev
```
